...
CreateThread(function()
	...
	plsr.Logger:Trace("Objects", string.format("Restored ^2%s^7 Persistant Props From Database", restored))
	...
	plsr.Chat:RegisterAdminCommand("addobj", function(source, args, rawCommand)
		plsr.Callbacks:ClientCallback(source, "Objects:StartPlacement", {
	...
	plsr.Chat:RegisterAdminCommand("addtobj", function(source, args, rawCommand)
		plsr.Callbacks:ClientCallback(source, "Objects:StartPlacement", {
			model = GetHashKey(args[1]),
			data = {
				model = args[1],
	...
	plsr.Chat:RegisterAdminCommand("deleteobj", function(source, args, rawCommand)
		plsr.Objects:Delete(source, tonumber(args[1]))
	end, {
		help = "Delete a Prop",
		params = {
			{
            ...
            if plyr.Permissions:IsStaff() or plyr.Permissions:IsAdmin() then
                ...
                if char ~= nil then
                    if _placedProps[id] ~= nil then
                        ...
                        plsr.Chat.Send.Server:Single(source, string.format("Unable To Locate Placed Prop With ID %s", id))
                    end
                else
                    return false
                end
...
RegisterNetEvent("Objects:Server:Create", function(data, endCoords)
	local src = source
	plsr.Objects:Create(src, data.type, data.isTemp, data.model, endCoords.coords, endCoords.rotation, data.isFrozen, data.nameOverride)
end)

RegisterNetEvent("Objects:Server:Delete", function(id)
	local src = source
	plsr.Objects:Delete(src, id)
end)

RegisterNetEvent("Objects:Server:View", function(id)
	local src = source
	plsr.Objects:Info(src, id)
end)
